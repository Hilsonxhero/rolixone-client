...
const fetchData = async () => {
    try {
        const { data } = await ApiService.get(`management/users/${route.params.id}`);

        form.value.username = data.data.username;
        form.value.status = data.data.status;
        form.value.role = data.data.role;
        form.value.is_superuser = data.data.is_superuser ? true : false;
        loading.value = false;
    } catch (error) { }
};
...
const handleSubmit = async () => {
    try {
        ...
        const { data } = await ApiService.put(`management/users/${route.params.id}`, formData);
        if (data.status == "200") {

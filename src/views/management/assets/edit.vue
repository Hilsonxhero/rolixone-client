...
const handleSubmit = async () => {
    try {
        ...
        const { data } = await ApiService.put(`management/assets/${route.params.id}`, formData);
...
const fetchData = async () => {
    try {
        const { data } = await ApiService.get(`management/assets/${route.params.id}`);

        form.value.name = data.data.name;
        form.value.symbol = data.data.symbol;
        form.value.network = data.data.network;
        form.value.min_withdraw = data.data.min_withdraw;
        form.value.max_withdraw = data.data.max_withdraw;
        form.value.min_deposit = data.data.min_deposit;
        form.value.max_deposit = data.data.max_deposit;
        form.value.status = data.data.status;
        form.value.type = data.data.type;
        form.value.validation_address = data.data.validation_address;
        form.value.can_withdraw = data.data.can_withdraw ? true : false;
        form.value.can_deposit = data.data.can_deposit ? true : false;
        form.value.icon_preview = data.data.media?.icon
        loading.value = false;
    } catch (error) { }
};
                        ...
                        <Form @submit="handleSubmit" ref="formRef">
                            <div class="grid grid-cols-12 gap-3">
                                ...
                                <div class="col-span-12">
                                    ...
                                    <template v-if="form.icon_preview">
                                        <div>
                                            <img :src="form.icon_preview">
                                        </div>
                                    </template>
